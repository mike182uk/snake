...
class Game extends EventEmitter {
  ...
  start (speed) {
    ...
    // Set random extension location
    this._extensionCoords = this._getRandomAvailableCoords()
  ...
  _nextTick () {
    ...
    // Check for extension
    if (nextRow === this._extensionCoords[0] && nextCol === this._extensionCoords[1]) {
      // Do not remove the tail so that the size of the snake increases
      removeTail = false

      // Update position of the extension
      this._extensionCoords = this._getRandomAvailableCoords()
    }
  ...
  _getRandomAvailableCoords () {
    ...
    // Ensure extension is not placed on snake
  ...
  _generateMatrix () {
    ...
    matrix.setValue(this._extensionCoords[0], this._extensionCoords[1], this._extensionValue)
